...
def convolve_rho_E(rho1, rho2):
    ...
    for e in Etot:
        ...
        e_minus_Eint = np.array(e-Eint).round(decimals=1)
        integrand = rho1[Eint].values * rho2[e_minus_Eint].values
...
class ped_models:
    ...
    def rovib_dos2(self):
        ...
        Emax = max(self.ped_df[self.ped_df.columns[-1]]
                   [self.ped_df.sort_index().index[-1]].index)
        # just to save stuff, remove later
        Evect_full = np.linspace(0, Emax, num=round((Emax-0)/0.1))
        ...
        for P in self.ped_df.columns:
            ...
            for T in self.ped_df.sort_index().index:
                ...
                upp_bound = min(E_dos0[-1], E[-1])
                E = Evect_full[Evect_full <= upp_bound]
                # series with the energy: otherwise, you have troubles with indexing
                # estraggo ped, rhovib, rhotrasl
                rho_trasl = dos_trasl(
                    self.mw_dct[self.prod1], self.mw_dct[self.prod2], E, P*101325, T)
                f_rho_trasl = interp1d(E, rho_trasl.values, kind='cubic',
                                       fill_value='extrapolate')

                P_E1E = pd.DataFrame(index=E, columns=E[3:], dtype=float)
                P_E1 = pd.Series(index=E[1:-2], dtype=float)
                #P_E1E = pd.DataFrame(columns=np.arange(3, len(E), dtype=int), dtype=float)
                #P_E1 = pd.Series(np.arange(1, len(E)-2, dtype=int), dtype=float)
                # idx = 3
                for e in E[3:]:

                    # you are fixing e, so E1 can be only smaller than E
                    E1 = E[E <= e]
                    E_minus_E1 = np.array(e-E1)
                    # compute the convolution between rho1 and rhonon1
                    # int(rho1(E1)*rho_non1(E-E1)dE1)

                    rho1_E1 = f_rho_rovib_prod1(E1)
                    rhonon1_E1 = convolve_f(
                        f_rho_rovib_prod2, f_rho_trasl, E_minus_E1)
                    rho1_rhonon1 = rho1_E1*rhonon1_E1
                    rhoE1tot = np.trapz(rho1_rhonon1, x=E1)
                    ...
                    P_E1E[e][E1] = P_E1Etot
                    #print(e, E1, P_E1E)
                    #P_E1E[idx][np.arange(1, len(E1), dtype=int)] = P_E1Etot
                # for each E1: extract P(E1;E) at different E and integrate
                # compute P(E1): you need to derive Pped[Eint]

                for E1 in E[1:-2]:
                    Evect = E[3:][E[3:] > E1]
                    P_E1Evect = P_E1E.loc[E1][Evect].values*f_ped_E(Evect)
                    P_E1[E1] = np.trapz(P_E1Evect, x=Evect)
    ...
    def rovib_dos_old(self):
        ...
        """
        # checks on input
        if self.dos_df == None or self.mw_dct == None:
            print('*Error: dos or mw not defined, exiting now \n')
            sys.exit()
        ...
        for P in self.ped_df.columns:
            ...
            P_E1_df = pd.DataFrame(
                index=np.arange(0.0, round(Emax, 1), 0.5).round(
                    decimals=1), columns=self.ped_df.sort_index().index, dtype=float)
            # just to save stuff, remove later
            ...
            for T in self.ped_df.sort_index().index:
                ...
                # riduci valori di E, E_dos al range minimo
                upp_bound = min(E_dos0[-1], E[-1])
                E = np.arange(0.0, round(upp_bound, 1), 0.1).round(
                    decimals=1)  # default step size 0.1
                # estraggo ped, rhovib, rhotrasl
                # technically the "extrapolate" shouldn't be necessary
                rho_rovib_prod1 = pd.Series(
                    f_rho_rovib_prod1(E), index=E)  # series
                rho_rovib_prod2 = pd.Series(
                    f_rho_rovib_prod2(E), index=E)  # series
                rho_trasl = dos_trasl(
                    self.mw_dct[self.prod1], self.mw_dct[self.prod2], E, P*101325, T)

                E1_vect = np.copy(E[::5])  # coarser
                ...
                for E1 in E1_vect[1:-1]:
                    ...
                    # calcola rho(E-E1)
                    E_minus_E1 = np.array(Eint-E1).round(decimals=1)
                    ...
                    # print(E1, rho1_E1, rho_non1_E_minus_E1)
                    # compute the convolution between rho1 and rhonon1
